...
function RegistrationStep1 () {
  ...
  const handleRegistrationStep1 = e => {
    ...
    userPool.signUp(email, password, [], null, function (err, result) {
      ...
      // result.userSub, result.userConfirmed, result.username
  ...
  const handleVerifyCode = e => {
    ...
    cognitoUser.confirmRegistration(verificationCode, true, (err, result) => {
      if (err) {
        ...
        // Handle verification error and provide appropriate feedback to the user
      } else {
        console.log(result)
        navigate('/registration-step2/' + user_id, {
          email: username,
          user_id: user_id
        })
        // Redirect to a success page or perform additional actions upon successful verification
      }
    })
  }
